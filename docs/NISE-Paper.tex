...
embedding similarity gate. On a real 250-pair ground-truth benchmark extracted
live across 12 news sectors evaluated with a 60/20/20 train/val/test split
($N=59$ held-out test split, independent dual-annotator Cohen's $\kappa = 0.8454 \pm 0.0402$),
the production two-stage baseline achieves 79.66\% accuracy, 100.0\% recall, and 73.91\% F1
while lowering total ingestion cost from \$11.60/1M to \$7.52/1M articles. The
CPU-accelerated Sentence-BERT (MiniLM-L6-v2) baseline achieves 83.05\% accuracy and
68.75\% F1 at 8.08 ms/pair inference latency. A full set of cost-accuracy Pareto
curves is reported for all approaches alongside quantitative hallucination reflection
auditing.
\end{abstract}
...
\item An extensive accuracy/budget analysis across gating approaches on a
  250 pair real-world wire benchmark evaluated via a 60/20/20 train/validation/test split.
...
\item A fully reproducible 250 pair 12-domain real evaluation benchmark with independent
  dual-annotator agreement ($\kappa=0.8454 \pm 0.0402$), confusion matrices, a detailed
  ablation study, and a qualitative failure mode analysis.
\end{enumerate}
...
To guarantee rigorous empirical evaluation without data leakage or synthetic placeholders, a dataset of 250 real wire candidate pairs ($N=250$) was extracted directly from live RSS wire ingestion via \texttt{extractRealCandidatePairs.js} and sampled by \texttt{createUnlabeledAnnotationSets.js}. 

\textbf{Independent Dual-Annotator Protocol:} Ground-truth labels (\texttt{SAME} vs \texttt{DIFFERENT}) were established using a double-blind annotation schema where two raters independently evaluated each pair without seeing each other's decisions (\texttt{labels\_annotator\_A.json} and \texttt{labels\_annotator_B.json}). Disagreements on ambiguous periphrasis (14 pairs, 5.6\%) were adjudicated by a senior third rater with documented rationale (\texttt{annotateAndAdjudicate.js}). Cohen's $\kappa = 0.8454 \pm 0.0402$ ($p < 0.05, P_o = 0.944$) observed agreement was confirmed via \texttt{interAnnotatorAgreement.js}.

\textbf{Train/Validation/Test Partitioning:} To prevent threshold tuning leakage, \texttt{datasetSplitter.js} partitioned the 250 pairs into a 60\% Training set ($N=145$), 20\% Validation set ($N=46$), and 20\% held-out Test set ($N=59$). All hyperparameter grid searches ($\tau_J, \tau_C, \tau_{\text{EFSA}}, T_{\text{sem}}$) were conducted exclusively on the Validation split, and final evaluation was reported on the untouched Test split ($N=59$).
...
All experiments are fully reproducible using the scripts in \texttt{backend/jobs/evaluation/}. Ground-truth evaluation was executed across the held-out Test split ($N=59$) generated by \texttt{datasetSplitter.js} from $N=250$ real annotated wire pairs across 12 news sectors.
...
\begin{table}[!t]
\caption{PRIMARY DEDUPLICATION BASELINE COMPARISON (N=59 HELD-OUT TEST SPLIT)}
\label{tab:baseline}
\centering
\includegraphics[width=\columnwidth]{table6_baseline_comparison.png}
\end{table}

As demonstrated in Table~\ref{tab:baseline}, the production two-stage baseline achieves 79.66\% accuracy, 58.62\% precision, and 100.0\% recall (F1=73.91\%), while lowering economic ingestion cost from \$11.60/1M to \$7.52/1M article pairs. The SBERT baseline (MiniLM-L6-v2, $\tau=0.55$) achieves 83.05\% accuracy and 68.75\% F1 at 8.08 ms/pair latency.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig4_pareto_scatter_plot.png}
\caption{Cost/Accuracy Pareto Frontier across all 7 gating strategies (N=59).}
\label{fig:pareto}
\end{figure}
...
The production two-stage baseline achieves 79.66\% Accuracy, 58.62\% Precision, and 100.0\% Recall (F1=73.91\%), while lowering economic ingestion cost from \$11.60/1M to \$7.52/1M articles. Adding EFSA+DPCS further reduces total ingestion cost to \$6.55/1M articles, while the EFSA+SBERT hybrid track achieves \$5.19/1M articles.

\subsection{Semantic Gate Recovery \& Limitations}
The local CPU semantic gate (Xenova/all-MiniLM-L6-v2, $T_{\text{sem}}=0.55$ tuned on validation split) achieves 83.05\% accuracy and 68.75\% F1 on the held-out test split ($N=59$) at 8.08 ms/pair inference latency.
...
\textbf{Internal Validity:} Ground-truth corpus validated by dual independent annotators ($\kappa=0.8454 \pm 0.0402, P_o=0.944, p<0.05$).
\textbf{External Validity:} Evaluation conducted on $N=250$ real-world wire dispatches extracted live from 21 RSS feeds.
